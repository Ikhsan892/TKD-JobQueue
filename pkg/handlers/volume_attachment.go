...
func HandlerVolumeAttachment(queue rmq.Queue, db *gorm.DB, cfg *configs.Config) {
	// maximum 5 worker or more
	for i := 0; i < 5; i++ {
		name := fmt.Sprintf("worker Volume Attachment consumer %d", i)
		if _, err := queue.AddConsumer(name, functions.NewVolume(&functions.Volume{
			WorkerIndex:   i,
			Config:        cfg,
